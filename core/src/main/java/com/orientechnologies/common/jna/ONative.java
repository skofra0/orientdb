...
public class ONative {
      ...
      if (result == 0 && rlimit.rlim_cur > 0) {
        if (verbose) {
          OLogManager.instance()
              .infoNoDb(this, "Detected limit of amount of simultaneously open files is %d, " + " limit of open files for disk cache will be set to %d", rlimit.rlim_cur, rlimit.rlim_cur / 2 - 512);
        }
   ...
   *         OrientDB instance is running
   *         inside container. If <code>null</code> is returned then it was impossible to detect
   *         amount of memory on machine.
   */
  public MemoryLimitResult getMemoryLimit(final boolean printSteps) {
    ...
    if (printSteps) {
      OLogManager.instance().infoNoDb(this, "%d B/%d MB/%d GB of physical memory were detected on machine", memoryLimit, convertToMB(memoryLimit), convertToGB(memoryLimit));
    }
    ...
    if (Platform.isLinux()) {
      ...
      if (result == 0) {
        if (printSteps)
          OLogManager.instance().infoNoDb(this, "Soft memory limit for this process is set to %d B/%d MB/%d GB", rlimit.rlim_cur, convertToMB(rlimit.rlim_cur), convertToGB(rlimit.rlim_cur));

        memoryLimit = updateMemoryLimit(memoryLimit, rlimit.rlim_cur);

        if (printSteps)
          OLogManager.instance().infoNoDb(this, "Hard memory limit for this process is set to %d B/%d MB/%d GB", rlimit.rlim_max, convertToMB(rlimit.rlim_max), convertToGB(rlimit.rlim_max));
      ...
      if (memoryCGroupPath != null) {
        ...
        if (!memoryCGroup.exists()) {
          if (printSteps)
            OLogManager.instance().infoNoDb(
                this, "Can not find '%s' path for memory cgroup, it is supposed that " + "process is running in container, will try to read root '%s' memory cgroup data", memoryCGroup,
                memoryCGroupRoot);
    ...
    if (printSteps) {
      if (memoryLimit > 0)
        OLogManager.instance().infoNoDb(this, "Detected memory limit for current process is %d B/%d MB/%d GB", memoryLimit, convertToMB(memoryLimit), convertToGB(memoryLimit));
      else
        ...
        try (final BufferedReader bufferedMemoryLimitReader = new BufferedReader(memoryLimitReader)) {
          try {
            ...
            try {
              ...
              if (printSteps)
                OLogManager.instance().infoNoDb(this, "cgroup soft memory limit is %d B/%d MB/%d GB", cgroupMemoryLimitValue, convertToMB(cgroupMemoryLimitValue), convertToGB(cgroupMemoryLimitValue));
        ...
        try (final BufferedReader bufferedMemoryLimitReader = new BufferedReader(memoryLimitReader)) {
          try {
            ...
            try {
              ...
              if (printSteps)
                OLogManager.instance().infoNoDb(this, "cgroup hard memory limit is %d B/%d MB/%d GB", cgroupMemoryLimitValue, convertToMB(cgroupMemoryLimitValue), convertToGB(cgroupMemoryLimitValue));
      ...
      try {
        ...
        try (final BufferedReader bufferedCGroupReader = new BufferedReader(cgroupListReader)) {
          ...
          try {
            ...
            OLogManager.instance()
                .errorNoDb(this, "Error during reading of details of list of cgroups for the current process, " + "no restrictions applied by cgroups will be taken into account", ioe);
            memoryCGroupPath = null;
          }

        } catch (final IOException e) {
          OLogManager.instance().errorNoDb(this, "Error during closing of reader which reads details of list of cgroups for the current process", e);
        }
      } catch (final FileNotFoundException fnfe) {
